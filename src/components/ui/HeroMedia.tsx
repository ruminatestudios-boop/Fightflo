...
interface HeroMediaProps {
  ...
  className?: string;
  /** Intro hero — preload full video and reveal as soon as frames load */
  eager?: boolean;
}
...
export function HeroMedia({
  ...
  useEffect(() => {
    ...
    const fallback = window.setTimeout(() => setVideoReady(true), eager ? 800 : 2000);

    video.play().catch(() => {
      /* autoplay blocked — still show first frame / poster */
      setVideoReady(true);
    });
  ...
  return (
    <div className={`hero-media-root absolute inset-0 overflow-hidden bg-black ${className}`}>
      {/* Cinematic base — always visible even if image/video fail */}
